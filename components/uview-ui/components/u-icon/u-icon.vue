...
<script>
import validation from '../../libs/function/test.js'
...
export default {
	...
	computed: {
		customClass() {
			...
			// 主题色，通过类配置
			if (this.showDecimalIcon && this.inactiveColor && config.type.includes(this.inactiveColor)) {
				classes.push('u-icon__icon--' + this.inactiveColor)
			} else if (this.color && config.type.includes(this.color)) classes.push('u-icon__icon--' + this.color)
		...
		iconStyle() {
			...
			} else if (this.color && !config.type.includes(this.color)) style.color = this.color
